...
let bullets = [];
const playerSpeed = 5;
const playerRadius = 15; // Aumentar o raio do jogador
...
document.addEventListener('click', (event) => {
  if (players[socket.id]) {
    ...
    const bullet = {
      x: players[socket.id].x, // A posição atual do jogador
    ...
    bullets.push(bullet);
    socket.emit('shootBullet', bullet); // Enviar a nova bala ao servidor
  }
});
...
function gameLoop() {
  if (players[socket.id]) {
    ...
    players[socket.id].x = Math.max(playerRadius, Math.min(players[socket.id].x, mapWidth - playerRadius));
    players[socket.id].y = Math.max(playerRadius, Math.min(players[socket.id].y, mapHeight - playerRadius));
  ...
  for (let id in players) {
    ...
    context.beginPath();
    context.arc(player.x, player.y, playerRadius, 0, 2 * Math.PI); // Usar playerRadius para o tamanho da seta
    context.fill();
    context.fillStyle = 'red';
    context.fillText(player.health, player.x - 10, player.y - 20); // Renderiza a saúde acima do jogador
  }
